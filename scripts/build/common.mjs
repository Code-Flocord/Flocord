...
export const gitHash = process.env.FLOCORD_HASH || execSync("git rev-parse HEAD", { encoding: "utf-8" }).trim();
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            ...
            for (const dir of pluginDirs) {
                ...
                for (const file of files) {
                    ...
                    metaCode += `[${mod}.name]:${JSON.stringify({ folderName })},\n`;
                    i++;
                }
            }
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.FLOCORD_REMOTE;
            if (!remote) {
...
export const commonOpts = {
    ...
    jsx: "transform",
    jsxFactory: "FlocordCreateElement",
    jsxFragment: "FlocordFragment"
};
