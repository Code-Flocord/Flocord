...
const commonOptions = {
    ...commonOpts,
    entryPoints: ["browser/Flocord.ts"],
    format: "iife",
    globalName: "Flocord",
...
const buildConfigs = [
    ...
    },
    {
        ...
        footer: { js: "//# sourceURL=file:///FlocordWeb" }
    },
    {
        ...commonOptions,
    ...
    },
    {
        ...
        outfile: "dist/Flocord.user.js",
        banner: {
            js: readFileSync("browser/userscript.meta.js", "utf-8").replace("%version%", `${VERSION}.${new Date().getTime()}`)
        },
        footer: {
            // UserScripts get wrapped in an iife, so define Flocord prop on window that returns our local
            js: "Object.defineProperty(unsafeWindow,'Flocord',{get:()=>Flocord});"
        }
    }
];
...
async function buildExtension(target, files) {
    const entries = {
        "dist/Flocord.js": await readFile("dist/browser/extension.js"),
        "dist/Flocord.css": await readFile("dist/browser/extension.css"),
...
const appendCssRuntime = readFile("dist/Flocord.user.css", "utf-8").then(content => {
    const cssRuntime = `unsafeWindow._vcUserScriptRendererCss=\`${content.replaceAll("`", "\\`")}\``;

    return appendFile("dist/Flocord.user.js", cssRuntime);
});
