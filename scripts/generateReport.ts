...
async function printReport() {
    ...
    console.log("# Flocord Report" + (CANARY ? " (Canary)" : ""));
    ...
    if (process.env.WEBHOOK_URL) {
        // Discord limits: 4096 chars per embed description, 6000 chars per message, 10 embeds
        const clamp = (s: string, max: number) => s.length > max ? s.slice(0, max - 20) + "\n… (truncated)" : s;
        ...
        for (const embed of embeds.slice(0, 10)) {
            if (!embed || !("description" in embed) || typeof embed.description !== "string") continue;
            embed.description = clamp(embed.description, Math.max(200, Math.min(4000, budget)));
            budget -= embed.description.length + (("title" in embed && embed.title) ? String(embed.title).length : 0);
        }

        const body = JSON.stringify({
            username: "Flocord Reporter" + (CANARY ? " (Canary)" : ""),
...
page.on("console", async e => {
    ...
    const isFlocord = firstArg === "[Flocord]";
