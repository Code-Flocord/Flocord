...
function makeShortcuts() {
    ...
    return {
        ...
        wpPatcher: { getter: () => Flocord.WebpackPatcher },
        wpInstances: { getter: () => Flocord.WebpackPatcher.allWebpackInstances },
        wpsearch: search,
        wpex: extract,
        ...
        PluginsApi: { getter: () => Flocord.Plugins },
        plugins: { getter: () => Flocord.Plugins.plugins },
        Settings: { getter: () => Flocord.Settings },
        Api: { getter: () => Flocord.Api },
        Util: { getter: () => Flocord.Util },
            ...
            if (!win.prepared) {
                ...
                [...document.querySelectorAll("style"), ...document.querySelectorAll("link[rel=stylesheet]")].forEach(s => {
                    ...
                    if (s.parentElement?.tagName === "HEAD")
                        doc.head.append(n);
                    else if (n.id?.startsWith("flocord-") || n.id?.startsWith("vcd-"))
        ...
        preEnable: (plugin: string) => (Flocord.Settings.plugins[plugin] ??= { enabled: true }).enabled = true,
...
function loadAndCacheShortcut(key: string, val: any, forceLoad: boolean) {
    ...
    function unwrapProxy(value: any) {
        ...
        } else if (value.$$flocordGetWrappedComponent) {
            return forceLoad ? value.$$flocordGetWrappedComponent() : value;
        }
...
export default definePlugin({
    ...
    start() {
        ...
        if (!IS_WEB) {
            const Native = FlocordNative.pluginHelpers.ConsoleShortcuts as PluginNative<typeof import("./native")>;
