            ...
            replacement: [
                {
                    ...
                    replace: "($&||!!Flocord.Api.NicknameIcons._renderIcons({userId:arguments[0].user?.id})?.length)"
                },
                {
                    match: /(?<=shouldUnderlineOnHover:null.{0,300})children:(\i)(?=\}\)\])/,
                    replace: "children:[...Flocord.Api.NicknameIcons._renderIcons({userId:arguments[0].user?.id}),$1]"
                }
            ]
        }
    ]
