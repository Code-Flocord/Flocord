...
/**
 * re-applies the flocord patch to a freshly installed discord host
...
interface DiscordUpdaterModule {
    ...
    __flocordTryInitWrapped?: boolean;
}
...
interface DiscordDesktopCore {
    ...
    __flocordStartupWrapped?: boolean;
}

const error = (...args: unknown[]) => console.error("[Flocord:HostUpdate]", ...args);
...
const retainFlocord = (updater: DiscordHostUpdater, reason: string) => {
    try {
...
const attachToUpdater = (updater: DiscordHostUpdater | null | undefined) => {
    ...
    updater.on?.("host-updated", () => retainFlocord(updater, "host-updated"));
    ...
    if (typeof sync === "function") {
        ...
        updater.startCurrentVersionSync = (options?: StartCurrentVersionOptions) => {
            bound(options);
            try { retainFlocord(updater, "startCurrentVersionSync"); } catch (e) { error(e); }
        };
    }
    ...
    if (typeof async_ === "function") {
        ...
        updater.startCurrentVersion = async (queryOptions?: object, options?: StartCurrentVersionOptions) => {
            ...
            try { retainFlocord(updater, "startCurrentVersion"); } catch (e) { error(e); }
        };
    }
};

const wrapStartup = (coreExports: DiscordDesktopCore | null | undefined) => {
    if (!coreExports?.startup || coreExports.__flocordStartupWrapped) return;
    coreExports.__flocordStartupWrapped = true;
    ...
    coreExports.startup = function (opts, ...rest) {
        try {
            ...
            } else if (typeof updaterModule?.tryInitUpdater === "function" && !updaterModule.__flocordTryInitWrapped) {
                /*
                 * updater not yet constructed at startup time. wrap the
                 * factory so we attach once vanilla creates it.
                 */
                updaterModule.__flocordTryInitWrapped = true;
