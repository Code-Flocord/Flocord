...
export const DATA_DIR = process.env.FLOCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "FlocordData", suffix)
        : join(app.getPath("userData"), "..", "Flocord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Flocord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
