...
function createFlocordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Flocord",
            submenu: [
                ...
                {
                    label: cachedUpdateAvailable ? "Update Flocord" : "Check for Updates",
                    click: () => sendToRenderer(IpcEvents.TRAY_CHECK_UPDATES)
                },
                {
                    label: "Repair Flocord",
...
export function patchTrayMenu(): void {
    ...
    Menu.buildFromTemplate = function (template: MenuItemConstructorOptions[]) {
        const alreadyPatched = template.some(item => item.label === "Flocord");
        if (isTrayMenu(template) && !alreadyPatched) {
            const insertIndex = findInsertIndex(template);
            template.splice(insertIndex, 0, ...createFlocordMenuItems());
        }
