...
export async function compileUsercss(fileName: string) {
    const themeData = await FlocordNative.themes.getThemeData(fileName);
    ...
    if (!preprocessorFn) {
        UserCSSLogger.error("File", fileName, "requires preprocessor", preprocessor, "which isn't known to Flocord");
        return null;
    }

    // "vencord" is kept so existing UserCSS themes written for Vencord keep working
    const varsToPass = {
        flocord: "true",
        vencord: "true"
    };
