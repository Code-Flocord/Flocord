...
export async function checkForUpdates() {
    changes = await Unwrap(FlocordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(FlocordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(FlocordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(FlocordNative.updater.getRepo());
