...
import { forceServerListRerender } from "@flocordplugins/_utils/serverListRefresh";
...
export const Native = FlocordNative.pluginHelpers.PrivateBrowser as PluginNative<any>;
...
function restoreSavedPath() {
    if (savedPath && (window.location.pathname + window.location.search + window.location.hash) !== savedPath) {
        try {
            const WP = (Flocord as any).Webpack;
