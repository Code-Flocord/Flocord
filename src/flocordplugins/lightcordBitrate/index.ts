...
import { showToast as flocordShowToast, Toasts } from "@webpack/common";
...
export default definePlugin({
    ...
    showToast(message: string, type = Toasts.Type.SUCCESS) {
        flocordShowToast(message, type, { duration: 7000 });
    },
    ...
    installVoiceHooks(voiceModule: any) {
        ...
        if (!prototype) {
            this.showToast("[ADEM] Voice module not found. Join a voice channel, then reload Flocord.", Toasts.Type.FAILURE);
        ...
        logger.info("Flocord hooks installed.");
    },

    uninstallVoiceHooks() {
        if (!this.voiceModule?.prototype) return;

        const { prototype } = this.voiceModule;
