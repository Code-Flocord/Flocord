...
import flocordToolbox from "@flocordplugins/flocordToolbox";
...
function SpotifyActivityToggleButton({ iconForeground, hideTooltips, nameplate }: UserAreaRenderProps) {
    ...
    if (!isLoaded || !spotifyId || location !== "PANEL" && isPluginEnabled(flocordToolbox.name)) return null;
...
const settings = definePluginSettings({
    location: {
        ...
        options: [
            ...
            { label: "Flocord Toolbox", value: "TOOLBOX" }
        ],
        get hidden() {
            return !isPluginEnabled(flocordToolbox.name);
        }
    },
});
