...
const Native = FlocordNative.pluginHelpers.MediaPlayer as PluginNative<typeof import("./native")>;
        ...
        <div className="vc-mp-panel">
            <div className="vc-mp-main">
                ...
                <div className="vc-mp-info">
                    <div className="vc-mp-title" title={info.title}>{info.title || "Unknown"}</div>
                    ...
                    <div className="vc-mp-controls">
                        <button className="vc-mp-btn" onClick={() => control("previous")} title="Previous"><IconPrev /></button>
                        <button className="vc-mp-btn vc-mp-btn-play" onClick={() => control(isPlaying ? "pause" : "play")} title={isPlaying ? "Pause" : "Play"}>
                            {isPlaying ? <IconPause /> : <IconPlay />}
                        </button>
                        <button className="vc-mp-btn" onClick={() => control("next")} title="Next"><IconNext /></button>
                        {canVolume && (
...
export default definePlugin({
    name: "MediaPlayer",
    description: "Music controls in the bottom-left panel. Spotify through its local API (volume, HD art) plus SMTC for VLC, Chrome and more.",
    ...
    patches: [
        {
            ...
            replacement: {
                ...
                replace: "$self.PanelWrapper,{FlocordOriginal:$1,",
            },
        },
    ],

    PanelWrapper({ FlocordOriginal, ...props }: { FlocordOriginal: React.ComponentType<any>; [k: string]: any }) {
        return (
            <>
                <ErrorBoundary noop><MediaPlayerPanel /></ErrorBoundary>
                <FlocordOriginal {...props} />
            </>
        );
    },
});
