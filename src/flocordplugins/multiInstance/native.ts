...
function registerMediaPermissionsForSession(ses: Electron.Session): void {
    const allowed = ["media", "mediaKeySystem", "geolocation", "notifications", "fullscreen", "openExternal", "clipboard-read", "clipboard-sanitized-write"];
...
function setBadgeCount(count: number, win: BrowserWindow): void {
    if (process.platform !== "win32") {
        try { app.setBadgeCount(count); } catch { }
    }
    // On Windows, badge is handled via flashFrame (called by the caller)
}
