...
import { ChannelStore, createRoot,Menu, React, ReactDOM, RestAPI, UserStore } from "@webpack/common";
    ...
    return ReactDOM.createPortal(
        ...
        >
            ...
            <div className="vc-sv-messages">
                {loading && <div className="vc-sv-state">Loading…</div>}
                {!loading && messages.length === 0 && (
                    <div className="vc-sv-state">No messages.</div>
                )}
                {messages.map(msg => {
...
function makeMenuPatch(getChannelId: (props: any) => string | undefined): NavContextMenuPatchCallback {
    return (children, props) => {
        ...
        children.push(
            <Menu.MenuItem
                id="vc-split-view"
                label={splitChannelId === cid ? "Close split view" : "Open in split view"}
...
export default definePlugin({
    name: "SplitView",
    description: "Opens a second conversation in a floating window you can drag and resize.",
