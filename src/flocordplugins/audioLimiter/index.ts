...
function applyLimiter(data: StreamData) {
    ...
    const UserStore = (window as any).Flocord?.Webpack?.findByStoreName?.("UserStore") || (window as any).Flocord?.Webpack?.common?.UserStore;
    ...
    patches: [
        {
            ...
            replacement: {
                // set volume(e){this._volume=Math.max(...),this.updateAudioElement()}
                match: /(?<=set volume\(\i\)\{this\._volume=.{0,80}?),this\.updateAudioElement\(\)/,
