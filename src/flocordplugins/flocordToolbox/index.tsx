...
import { useShouldHideToolbox } from "@flocordplugins/streamProof";
...
function ToolboxButton() {
    ...
    const hidden = useShouldHideToolbox();
        ...
        >
            {(_, { isShown }) => (
                <HeaderBarButton
                    ...
                    onClick={() => setShow(v => !v)}
                    tooltip={isShown ? null : "Flocord Toolbox"}
                    icon={Icon}
                    selected={isShown}
                />
            )}
...
migratePluginSettings("FlocordToolbox", "EquicordToolbox", "VencordToolbox");
export default definePlugin({
    name: "FlocordToolbox",
    description: "Adds a button next to the inbox button in the channel header that houses Flocord quick actions",
