...
export const styleMap = window.FlocordStyles ??= new Map();

export const flocordRootNode = document.createElement("flocord-root");
/**
 * Houses all Flocord core styles. This includes all imported css files
 */
export const coreStyleRootNode = document.createElement("flocord-styles");
/**
 * Houses all plugin specific managed styles
 */
export const managedStyleRootNode = document.createElement("flocord-managed-styles");
/**
 * Houses the user's themes and quick css
 */
export const userStyleRootNode = document.createElement("flocord-user-styles");

flocordRootNode.style.display = "none";
flocordRootNode.append(coreStyleRootNode, managedStyleRootNode, userStyleRootNode);

export function initStyles() {
    const osValuesNode = createAndAppendStyle("flocord-os-theme-values", coreStyleRootNode);
    createAndAppendStyle("flocord-text", coreStyleRootNode).textContent = generateTextCss();
    const rendererCssNode = createAndAppendStyle("flocord-css-core", coreStyleRootNode);
    const vesktopCssNode = (IS_VESKTOP || IS_EQUIBOP) ? createAndAppendStyle("vesktop-css-core", coreStyleRootNode) : null;
    createAndAppendStyle("flocord-margins", coreStyleRootNode).textContent = generateMarginCss();

    FlocordNative.native.getRendererCss().then(css => rendererCssNode.textContent = css);
    if (IS_DEV) {
        FlocordNative.native.onRendererCssUpdate(newCss => {
    ...
    FlocordNative.themes.getSystemValues().then(values => {
...
document.addEventListener("DOMContentLoaded", () => {
    document.documentElement.append(flocordRootNode);
}, { once: true });
...
export function enableStyle(name: string) {
    ...
    if (!style.dom) {
        ...
        style.dom.dataset.flocordName = style.name;
    }
    compileStyle(style);
