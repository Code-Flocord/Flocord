...
import { ApplicationCommandInputType, ApplicationCommandOptionType, ApplicationCommandType, FlocordCommand } from "./types";

export * from "./commandHelpers";
export * from "./types";

export let BUILT_IN: FlocordCommand[];
export const commands = {} as Record<string, FlocordCommand>;
...
export const _init = function (cmds: FlocordCommand[]) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd: FlocordCommand, args: CommandArgument[], ctx: CommandContext) {
    if (!cmd.isFlocordCommand)
 ...
 * @param opt
 */
export function prepareOption<O extends CommandOption | FlocordCommand>(opt: O): O {
...
// investigate
function registerSubCommands(cmd: FlocordCommand, plugin: string) {
...
export function registerCommand<C extends FlocordCommand>(command: C, plugin: string) {
    if (!BUILT_IN) {
        console.warn(
            "[CommandsAPI]",
