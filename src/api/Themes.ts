...
import { coreStyleRootNode, flocordRootNode,managedStyleRootNode, userStyleRootNode } from "./Styles";
...
async function toggle(isEnabled: boolean) {
    if (!style) {
        if (isEnabled) {
            style = createAndAppendStyle("flocord-custom-css", userStyleRootNode);
            FlocordNative.quickCss.addChangeListener(css => {
                style.textContent = css;
                // At the time of writing this, changing textContent resets the disabled state
                style.disabled = !Settings.useQuickCss;
                updatePopoutWindows();
            });
            style.textContent = await FlocordNative.quickCss.get();
        }
    } else
...
async function initThemes() {
    themesStyle ??= createAndAppendStyle("flocord-themes", userStyleRootNode);
    ...
    if (IS_WEB) {
        for (const theme of enabledThemes) {
            ...
            const themeData = await FlocordNative.themes.getThemeData(theme);
        ...
        for (const theme of enabledThemes) {
            const mode = getThemeActivationMode(theme);
            if (!shouldApplyTheme(mode, activeTheme)) continue;
            links.add(`flocord:///themes/${theme}?v=${version}`);
        }
    }
...
function applyToPopout(popoutWindow: Window | undefined, key: string) {
    ...
    doc.querySelector("flocord-root")?.remove();

    const clonedRoot = flocordRootNode.cloneNode(false) as HTMLElement;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!IS_WEB) {
        FlocordNative.quickCss.addThemeChangeListener(initThemes);
    }
}, { once: true });
