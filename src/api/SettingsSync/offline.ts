    ...
    switch (type) {
        case "all": {
            ...
            if (parsed.settings) {
                ...
                await FlocordNative.settings.set(PlainSettings);
            }
            if (parsed.quickCss) await FlocordNative.quickCss.set(parsed.quickCss);
        ...
        case "css": {
            ...
            await FlocordNative.quickCss.set(parsed.quickCss);
            break;
        }
        case "datastore": {
...
export async function exportSettings({ syncDataStore = true, type = "all", minify }: { syncDataStore?: boolean; type?: BackupType; minify?: boolean; }) {
    const settings = FlocordNative.settings.get();
    const quickCss = await FlocordNative.quickCss.get();
    let dataStore: any;
...
export async function downloadSettingsBackup(type: BackupType = "all", { minify }: { minify?: boolean; } = {}) {
    try {
        ...
        const filename = `flocord-${type}-backup-${moment().format("YYYY-MM-DD")}.json`;
...
export async function uploadSettingsBackup(type: BackupType = "all", showToast = true): Promise<void> {
    if (IS_DISCORD_DESKTOP) {
        const [file] = await DiscordNative.fileManager.openFiles({
            filters: [
                { name: "Flocord Settings Backup", extensions: ["json"] },
                { name: "all", extensions: ["*"] }
            ]
        });
