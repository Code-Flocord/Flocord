    ...
    try {
        ...
        addPatch({
            ...
            replacement: {
                ...
                replace: "(Flocord.Webpack._initReporter(),$&)"
            }
        }, "Flocord Reporter");

        // @ts-expect-error
        Flocord.Webpack._initReporter = function () {
        ...
        for (const [searchType, args] of Webpack.lazyWebpackSearchHistory) {
            ...
            try {
                ...
                if (result == null || (result.$$flocordGetWrappedComponent != null && result.$$flocordGetWrappedComponent() == null)) throw new Error("Webpack Find Fail");
            } catch (e) {
                let logMessage = searchType;
                if (method === "find" || method === "proxyLazyWebpack" || method === "LazyComponentWebpack") {
                    if (args[0].$$flocordProps != null) {
                        logMessage += `(${args[0].$$flocordProps.map(arg => `"${arg}"`).join(", ")})`;
                    } else {
...
// Run after the Flocord object has been created.
// We need to add extra properties to it, and it is only created after all of Flocord code has ran
if (IS_REPORTER)
