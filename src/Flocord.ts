...
async function runUpdateCheck() {
    ...
    try {
        ...
        if (IS_DISCORD_DESKTOP) FlocordNative.tray.setUpdateState(isOutdated);
        ...
        showNotice(
            "A new version of Flocord is available!",
            "View Update",
...
function initTrayIpc() {
    ...
    FlocordNative.tray.onCheckUpdates(async () => {
        try {
            const isOutdated = await checkForUpdates();
            FlocordNative.tray.setUpdateState(isOutdated);

            if (isOutdated) {
                showNotice("An Flocord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    FlocordNative.tray.onRepair(async () => {
        try {
            await update();
            relaunch();
        } catch (err) {
            UpdateLogger.error("Failed to repair Flocord", err);
        }
    });

    FlocordNative.tray.setUpdateState(getIsOutdated);
}
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Flocord.",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (IS_DISCORD_DESKTOP && Settings.winNativeTitleBar && IS_WINDOWS) {
        createAndAppendStyle("flocord-native-titlebar-style", coreStyleRootNode).textContent = "[class*=titleBar]{display: none!important}";
    }
}, { once: true });
