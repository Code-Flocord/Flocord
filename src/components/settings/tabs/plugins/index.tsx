...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Flocord"
};
...
export default function PluginSettings() {
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!isPluginEnabled(plugin.name)) return false;
                break;
            case SearchStatus.FLOCORD:
                if (!PluginMeta[plugin.name].folderName.startsWith("src/flocordplugins/")) return false;
                break;
    ...
    const [newPluginsSet] = useAwaiter(() => DataStore.get("Flocord_existingPlugins").then((cachedPlugins: Record<string, number> | undefined) => {
        ...
        DataStore.set("Flocord_existingPlugins", existingTimestamps);
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                const tooltipText = p.required || !depMap[p.name]
                    ? "This plugin is required for Flocord to function."
            ...
            <div className={classes(Margins.bottom20, Margins.top8, cl("filter-controls"))}>
                <Select
                    options={[
                        ...
                        { label: "Show Flocord Exclusives", value: SearchStatus.FLOCORD },
