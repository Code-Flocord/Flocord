...
// Only import pure modules that don't import other parts of Flocord.
...
const setCssDebounced = debounce((css: string) => FlocordNative.quickCss.set(css));

const themeStore = DataStore.createStore("FlocordThemes", "FlocordThemeData");

// probably should make this less cursed at some point
window.FlocordNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Code-Flocord/Flocord" }),
    ...
    quickCss: {
        get: () => DataStore.get("FlocordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("FlocordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "FlocordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => FlocordNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("FlocordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("FlocordSettings", JSON.stringify(s)),
